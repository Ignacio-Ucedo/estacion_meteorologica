...
El backend FastAPI SHALL incluir `CORSMiddleware` configurado para aceptar peticiones desde `http://localhost:5173` y `http://127.0.0.1:5173` (orígenes del dev server de Vite) en el entorno de desarrollo.

#### Scenario: Petición cross-origin desde el frontend
- **WHEN** el frontend en `http://localhost:5173` o `http://127.0.0.1:5173` realiza un `fetch` a `http://localhost:8000/api/stations`
