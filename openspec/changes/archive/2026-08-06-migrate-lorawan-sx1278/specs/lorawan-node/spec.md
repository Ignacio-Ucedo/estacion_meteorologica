...
El firmware del nodo sensor SHALL inicializar el stack LMIC configurado para EU433 y completar el proceso de join OTAA con ChirpStack usando DevEUI, AppEUI y AppKey almacenados en NVS antes de comenzar el ciclo de lecturas y transmisiones.

#### Scenario: Activación OTAA exitosa en primer arranque
- **GIVEN** el nodo tiene DevEUI, AppEUI y AppKey correctos en NVS y ChirpStack está disponible en la red con band plan EU433
...
### Requirement: El nodo transmite uplinks binarios de campos fijos por LoRaWAN EU433

El firmware del nodo sensor SHALL transmitir uplinks no confirmados cada 10 minutos usando el stack LMIC configurado para EU433, canal fijo 433.175 MHz SF7BW125. El FRMPayload SHALL tener exactamente 14 bytes con la siguiente estructura en little-endian:
...
#### Scenario: El nodo respeta el duty cycle de EU433
- **GIVEN** el stack LMIC está configurado para EU433
- **WHEN** el nodo transmite uplinks cada 10 minutos
- **THEN** LMIC no bloquea la transmisión por duty cycle (un uplink de 14 bytes en SF7BW125 dura ~50 ms, equivalente a 0.008% de duty cycle en el intervalo de 10 min, muy por debajo del límite del 1% de EU433)
