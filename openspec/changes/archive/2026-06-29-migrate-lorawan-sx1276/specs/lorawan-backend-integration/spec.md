...
El repositorio SHALL incluir un `docker-compose.yml` en el directorio `infra/` que levante ChirpStack (network server + application server + base de datos), Mosquitto (broker MQTT) e InfluxDB, de modo que el entorno completo pueda iniciarse con un único comando.
...
- **THEN** ChirpStack, Mosquitto e InfluxDB están disponibles en los puertos configurados y FastAPI puede conectarse a todos
