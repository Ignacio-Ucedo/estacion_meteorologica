...
type MetricParams = {
  base: number;
  seasonalAmp: number;  // how much the mean swings across the year
  warmInSummer: boolean; // southern hemisphere: summer = Dec
  dailySpread: number;  // typical min-max spread in a single day
  noise: number;        // day-to-day random variation of the mean
  clampMin: number;
  clampMax: number;
  dec: number;
};
...
function generateDailySeries(metric: MetricKey, days: number): DailySummary[] {
  ...
  for (let i = 0; i < days; i++) {
    ...
    // Southern hemisphere: cold in June (i≈0 when days=365), warm in December
