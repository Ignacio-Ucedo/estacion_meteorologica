...
export function ChartCard({
  ...
  const peak = useMemo(() => {
    ...
    const h = maxHour % 24;
    return { value: maxVal, label: `${h.toString().padStart(2, "0")}:00 ART` };
  }, [data, dataKey]);
  ...
  function bandChart(dailyData: DailySummary[], xAxisEl: JSX.Element, showDots: boolean) {
    return (
      <ComposedChart data={dailyData} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
        ...
        />
        {/* Transparent base up to min — stacked so band starts at min */}
        <Area type="monotone" dataKey="min" stackId="band"
          stroke="none" fill="transparent" isAnimationActive={false} legendType="none" />
        {/* Colored band from min to max */}
        <Area type="monotone" dataKey="spread" stackId="band"
  ...
  return (
    <article className={`chart-card ${tone}`}>
      <div className="chart-card-head">
        <div>
          ...
          <p className="chart-card-subtitle">
            {period === "1D" ? subtitle :
             period === "7D" ? "Últimos 7 días — banda min/max y media" :
             period === "30D" ? "Últimos 30 días — banda min/max y media" :
             "Últimos 12 meses — banda min/max y media"}
          </p>
        </div>
      ...
      <div className="chart-card-controls">
        ...
        <button className="reset-scale-btn" type="button" onClick={() => setRange(activeIdealDomain)}>
          Auto
        </button>
      </div>
    </article>
